  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessages(prev => prev.map(msg => 
        msg.id === aiMessageId ? { ...msg, text, isLoading: false } : msg
      ));
      ...
      setMessages(prev => prev.map(msg => 
        msg.id === aiMessageId ? { ...msg, text: errorText, isLoading: false } : msg
      ));
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-4rem)] max-h-[calc(100vh-4rem)] bg-white dark:bg-slate-800 rounded-xl shadow-md">
      ...
      <form onSubmit={handleSend} className="p-4 border-t border-slate-200 dark:border-slate-700 flex items-center gap-2">
        ...
        <button 
          type="submit" 
          className="bg-violet-600 text-white rounded-full p-3 hover:bg-violet-700 disabled:bg-slate-400" 
